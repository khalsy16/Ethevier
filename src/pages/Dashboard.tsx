...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const iq = query(collection(db, 'users', user.uid, 'impulseChecks'), where('status', '==', 'considering'));
    const unsubscribeImpulse = onSnapshot(iq, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const stats = (() => {
    ...
    return {
      weekly: calcTotal(weekStart),
      monthly: calcTotal(monthStart),
      yearly: calcTotal(yearStart),
      total: incomes.reduce((acc, curr) => acc + (curr.amount || 0), 0)
    };
  })();
  ...
  if (loading) return <div className="p-8 text-xavier-blue animate-pulse">Memindai bintang-bintang...</div>;

  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-star-white mb-2">Halo, {user?.displayName}!</h2>
          <p className="text-xavier-blue/70 text-sm sm:text-base">Inilah orbit keuangan surgawimu hari ini.</p>
        </div>
        {finishedChecks.length > 0 && (
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest">Peringatan Impuls</p>
              <p className="text-sm font-semibold text-star-white">{finishedChecks.length} item butuh ditinjau</p>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {[
          { label: 'Rezeki Mingguan', value: stats.weekly, icon: TrendingUp, color: 'text-green-400' },
          { label: 'Rezeki Bulanan', value: stats.monthly, icon: Wallet, color: 'text-aether-gold' },
          { label: 'Rezeki Tahunan', value: stats.yearly, icon: CreditCard, color: 'text-xavier-blue' },
          { label: 'Total Tabungan', value: stats.total, icon: ArrowUpRight, color: 'text-star-white' }
        ].map((item, id) => (
          <motion.div 
            key={id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-celestial-depth/50 border border-white/10 rounded-[2.5rem] p-8">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-star-white">Aktivitas Galaksi Terakhir</h3>
            <button className="text-sm text-aether-gold font-semibold hover:underline">Lihat Semua</button>
          </div>
          <div className="space-y-4">
            {incomes.slice(0, 5).map((income, idx) => (
              <div key={income.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                <div className="w-10 h-10 rounded-full bg-aether-gold/20 flex items-center justify-center">
                  <ArrowUpRight className="w-5 h-5 text-aether-gold" />
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-star-white">{income.source || 'Tabungan Harian'}</p>
                  <p className="text-xs text-xavier-blue/60">{format((income.date as Timestamp).toDate(), 'PPP p')}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-green-400">+{formatIDR(income.amount)}</p>
                </div>
              </div>
            ))}
            {incomes.length === 0 && <p className="text-center py-8 text-xavier-blue/40 italic">Belum ada pendapatan yang tercatat.</p>}
          </div>
        </div>
        ...
        <div className="bg-aether-gold/10 border border-aether-gold/20 rounded-[2.5rem] p-8 flex flex-col">
          <div className="flex items-center gap-3 mb-6">
             ...
             <h3 className="text-xl font-bold text-aether-gold">Misi Saat Ini</h3>
          </div>
          
          {nextWishlist ? (
            <div className="flex-1 flex flex-col">
              <p className="text-sm text-xavier-blue/60 mb-2">Menabung untuk:</p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <p className="text-xavier-blue/50 italic mb-4">Kamu belum memiliki misi aktif.</p>
              <button 
      ...
      <div className="bg-celestial-depth/30 border border-white/5 rounded-[2.5rem] p-8">
        <h3 className="text-xl font-bold text-star-white mb-6">Catatan Mimpi & Tenggat Waktu</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               ...
               <div className="flex items-center justify-between">
                  ...
                  {item.deadline && (
                    <span className="text-[10px] text-red-400 font-bold bg-red-400/10 px-2 py-1 rounded">
                      Tenggat: {format(item.deadline.toDate(), 'dd MMM')}
                    </span>
                  )}
               </div>
               {item.notes && (
          ...
          {wishlist.filter(item => item.notes || item.deadline).length === 0 && (
            <p className="col-span-full text-center py-10 text-xavier-blue/30 italic">Belum ada catatan atau tenggat waktu untuk harapanmu.</p>
          )}
        </div>
      </div>
    </div>
