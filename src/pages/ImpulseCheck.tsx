...
export default function ImpulseCheck() {
  ...
  const calculateDaysNeeded = (itemPrice: number) => {
    if (dailyAverage <= 0) return '∞';
    return Math.ceil(itemPrice / dailyAverage);
  };

  if (loading) return <div className="p-8 text-xavier-blue">Menavigasi nebula...</div>;
    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-star-white">Perisai Impuls</h2>
          </div>
          <p className="text-xavier-blue/70">Lindungi galaksimu dari lubang hitam impulsif.</p>
        </div>
        <button 
      ...
      <div className="bg-celestial-depth/30 border border-white/5 p-6 rounded-[2rem] flex items-start gap-4">
        ...
        <p className="text-sm text-xavier-blue/60 leading-relaxed italic">
          "Perisai Impuls membantumu memutuskan apakah sebuah pembelian benar-benar diperlukan. Dengan menunggu beberapa hari, 
          kamu membiarkan 'keinginan' awal mereda, memberikan ruang bagi 'kebutuhan' untuk bersinar."
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                    ...
                    {check.status === 'considering' ? 'Menimbang' : check.status === 'avoided' ? 'Terhindari' : 'Terbeli'}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="bg-white/5 p-4 rounded-2xl">
                    <p className="text-[10px] text-xavier-blue uppercase tracking-widest font-bold mb-1">Usaha Rezeki</p>
                    <p className="text-sm font-bold text-star-white">{calculateDaysNeeded(check.price)} Hari</p>
                  </div>
                  <div className="bg-white/5 p-4 rounded-2xl">
                    <p className="text-[10px] text-xavier-blue uppercase tracking-widest font-bold mb-1">Masa Tunggu</p>
                    <p className="text-sm font-bold text-star-white">{isFinished ? 'Selesai' : `${remainingDays} Hari Lagi`}</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {isFinished && (
                      <div className="p-4 bg-aether-gold/10 border border-aether-gold/20 rounded-2xl text-center">
                        <p className="text-xs font-bold text-aether-gold uppercase tracking-widest animate-pulse">Waktu tunggu selesai! Masih butuh?</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {check.status === 'avoided' && (
                  <div className="flex items-center gap-3 text-green-400 font-bold justify-center py-2">
                    ...
                    <span>Perisai berhasil. Uang terselamatkan!</span>
                  </div>
                )}
              </motion.div>
            );
        ...
        {checks.length === 0 && (
          <div className="col-span-full py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
            ...
            <p className="text-xavier-blue/40 italic">Tidak ada lubang hitam impulsif terdeteksi.</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-0">
             ...
             >
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-star-white">{editingCheck ? 'Edit Perisai' : 'Cek Kepentingan'}</h3>
                ...
                <form onSubmit={handleAdd} className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black text-xavier-blue uppercase tracking-widest px-2">Nama Barang</label>
                      <input 
                        value={name}
                        onChange={e => setName(e.target.value)}
                        placeholder="Apa yang membuatmu tergoda?"
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black text-xavier-blue uppercase tracking-widest px-2">Harga (IDR)</label>
                      <input 
                        type="number"
                        value={price}
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black text-xavier-blue uppercase tracking-widest px-2">Waktu Tunggu (Hari)</label>
                      <input 
                        type="number"
                        value={waitDays}
                   ...
                   <button className="w-full bg-aether-gold text-celestial-dark font-black py-5 rounded-2xl text-lg hover:scale-[1.02] shadow-[0_10px_30px_rgba(244,208,111,0.2)] transition-all">
                      {editingCheck ? 'Perbarui Perisai' : 'Aktifkan Perisai'}
                   </button>
                </form>
             </motion.div>
          </div>
